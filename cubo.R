...
mi_data_3 <- mi_data_3 %>%
  mutate(
    # De manera similar a lo anterior, aqui como ya estaban definidos a los students solo lo estandarizamos para los que no aparecen como Students sean other buscando no tener diferentes dimeciones unicas
...
# Contamos por combinacion de variables
cubo_data <- mi_data %>%
  group_by(age, gender, employment_status, depression) %>%
...
cubo_data <- cubo_data %>%
  mutate(
    x = x_map[as.character(age)],
    y = y_map[gender],
    z = z_map[employment_status]
  )

# ---------------- Visualizacion 3D

# Convertimos la depresion en variable numerica para poder distenguirla por color
cubo_data <- cubo_data %>%
  mutate(
    depression_numeric = ifelse(depression == "Yes", 1, 0)
  )
...
fig <- plot_ly(
  ...
  marker = list(
    ...
    color = ~depression_numeric,   # usamos la variable numerica
    colorscale = list(c(0, "blue"), c(1, "red")),  # azul = No, rojo = Yes
    opacity = 0.8,
    sizemode = "diameter",
    showscale = TRUE               # mostramos la escala de color
  )
) %>%
  layout(
    scene = list(
    ...
    title = "Cubo MOLAP Extendido - Depresión por Edad, Género y Estado Laboral",
    margin = list(l = 0, r = 0, b = 0, t = 50),
    legend = list(
      title = list(text = "Depresión"),
      itemsizing = "constant"
    )
  )
